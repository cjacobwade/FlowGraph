...
public class GlobalEventDefinitionDrawer : PropertyDrawer
{
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		if (globalEventInfos.Count == 0)
		{
			EditorGUI.LabelField(position, new GUIContent("No GlobalEvents found."), EditorStyles.helpBox);
		}
		else
		{
				...
				if (typeProp != null && eventProp != null)
				{
					...
					using (var check = new EditorGUI.ChangeCheckScope())
					{
						...
						if (changedEvent || parameterInfos.Length != parametersProp.arraySize)
						{
							...
							{
								var parameter = EventParameterHelper.GetParameterOfType(parameterInfos[i].ParameterType);
