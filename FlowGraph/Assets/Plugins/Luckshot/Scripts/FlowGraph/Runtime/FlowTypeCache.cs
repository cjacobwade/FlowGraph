...
public static class FlowTypeCache
{
			...
			foreach (var type in assemblyTypes)
			{
				if (type.IsSubclassOf(typeof(PropertyItem)) || type == typeof(PropertyItem))
				{
					...
					foreach (var method in methods)
					{
						...
						if (!nameToMemberMap.ContainsKey(method.Name) &&
	...
	public static Dictionary<string, MemberInfo> GetPropertyItemMembers(Type type)
    {
		InitializeIfNeeded();

		if (propertyItemMemberInfoMap.TryGetValue(type, out Dictionary<string, MemberInfo> typeToMemberMap))
	...
	public static MemberInfo GetPropertyItemMember(Type type, string memberName)
    {
		...
		while (typeIter != null)
		{
			var propertyItemToMemberMap = GetPropertyItemMembers(typeIter);
			if (propertyItemToMemberMap != null)
			{
