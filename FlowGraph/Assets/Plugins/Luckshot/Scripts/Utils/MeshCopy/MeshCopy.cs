...
public class MeshCopy : MonoBehaviour
{
	...
	[SerializeField]
	private MeshRenderer overrideRenderSettings = null;
	...
	public void CopyFrom(Transform sourceRoot)
	{
		...
		for(int i = 0; i < renderers.Length; i++)
		{
			...
			if (sourceRend.GetComponent<MeshCopyExcluder>())
				continue;
			...
			Vector3 posOffset = sourceRoot.transform.InverseTransformPoint(sourceRend.transform.position);
			...
			{
				copyMR.shadowCastingMode = overrideRenderSettings.shadowCastingMode;
				copyMR.receiveShadows = overrideRenderSettings.receiveShadows;
			}
			else
			{
